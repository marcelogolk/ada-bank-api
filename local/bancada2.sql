...
-- Banco: banco_db
-- ============================================================
-- passwords: todos os usuários têm a password  ->  senha123
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_conta_customer    ON conta     (customer_id);
CREATE INDEX IF NOT EXISTS idx_trans_origem     ON transacao (source_account_id);
CREATE INDEX IF NOT EXISTS idx_trans_destino    ON transacao (destination_account_id);
CREATE INDEX IF NOT EXISTS idx_trans_data_hora  ON transacao (data_hora);
CREATE INDEX IF NOT EXISTS idx_trans_type       ON transacao (type);

-- ------------------------------------------------------------
-- Dados de exemplo
-- ------------------------------------------------------------

INSERT INTO customer (name, cpf, email, senha, role)
VALUES
...
INSERT INTO conta (numero, type, cliente_id)
VALUES
    ('0001-1', 'CORRENTE', 3),
    ('0002-2', 'POUPANCA', 4),
    ('0003-3', 'ELETRONICA', 5),
...
INSERT INTO transacao (type, valor, data_hora, source_account_id, destination_account_id)
VALUES
    -- Depósitos iniciais
...
SELECT type, COUNT(*) FROM conta GROUP BY type ORDER BY type;
SELECT COUNT(*) FROM transacao;
SELECT type, COUNT(*) FROM transacao GROUP BY type ORDER BY type;
